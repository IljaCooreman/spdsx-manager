import { objectToXml, parseXml, extractValues, objectKeyWithUnderscore, stringToDecimal, decimalToString } from "../src/classes/xmlUtils"
...
describe('xml utils', () => {
  it('can parse an xml to json', () => {
    expect(objectToXml(testObject)).toBe('<parent><Nm1>0</Nm1><Nm2>01</Nm2></parent>');
  })

  it('can parse an XML', () => {
    const string = io.localReadFile(`${__dirname}/mock/00.spd`);
    expect(parseXml(string)).toStrictEqual(expectedResult);
  })
